...
func main() {
	...
	err := h.Run(context.Background(), "027_research_assistant", runExample)
	if err != nil {
		log.Fatal(err)
	}
...
func runExample(ctx context.Context) (*dsgo.Prediction, *harness.ExecutionStats, error) {
	...
	if err != nil {
		return nil, stats, fmt.Errorf("ReAct forward failed: %w", err)
	}
	...
	fmt.Println("\n📝 SUMMARY:")
	fmt.Println(result.Outputs["summary"])

	fmt.Println("\n🔍 KEY FINDINGS:")
	fmt.Println(result.Outputs["key_findings"])

	if stats_data, ok := result.Outputs["statistics"]; ok && stats_data != nil {
		fmt.Println("\n📈 STATISTICS:")
		fmt.Println(stats_data)
	}

	fmt.Println("\n💡 RECOMMENDATIONS:")
	fmt.Println(result.Outputs["recommendations"])

	fmt.Println("\n📊 METADATA:")
	fmt.Printf("  Confidence Level: %v\n", result.Outputs["confidence_level"])
	fmt.Printf("  Research Quality: %v\n", result.Outputs["research_quality"])
	fmt.Printf("  Sources Consulted: %v\n", result.Outputs["sources_consulted"])
	...
	stats.Metadata["focus_areas"] = inputs["focus_areas"]
	stats.Metadata["depth_level"] = inputs["depth_level"]
	stats.Metadata["confidence_level"] = result.Outputs["confidence_level"]
	stats.Metadata["research_quality"] = result.Outputs["research_quality"]
	stats.Metadata["sources_consulted"] = result.Outputs["sources_consulted"]
