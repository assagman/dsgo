...
func NewPredict(signature *Signature, lm LM) *Predict {
	return &Predict{
		Signature: signature,
		LM:        lm,
...
func (p *Predict) WithOptions(options *GenerateOptions) *Predict {
	p.Options = options
	return p
}
...
func (p *Predict) parseOutput(content string) (map[string]interface{}, error) {
...
func (p *Predict) coerceTypes(outputs map[string]interface{}) map[string]interface{} {
	...
	for key, value := range outputs {
		...
		if field.Type == FieldTypeString || field.Type == FieldTypeClass {
