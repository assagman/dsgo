...
func withStructuredOutputs(t *testing.T, enabled bool) func() {
	...
	if err := core.Configure(core.WithStructuredOutputEnabled(enabled)); err != nil {
		t.Fatalf("Configure: %v", err)
	}
	return func() {
		if err := core.Configure(core.WithStructuredOutputEnabled(originalSettings.StructuredOutput.Enabled)); err != nil {
			t.Fatalf("Configure: %v", err)
		}
	}
}
