...
// TestPredictionGetStringStripMarkers tests that field markers are stripped in GetString
func TestPredictionGetStringStripMarkers(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		expected string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pred := NewPrediction(map[string]any{
				"test": tt.input,
			})

			result, ok := pred.GetString("test")
			if !ok {
