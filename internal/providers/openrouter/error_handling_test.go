...
func TestGenerate_WithTools_RequestStructure(t *testing.T) {
	...
	opts := &core.GenerateOptions{Tools: tools}
...
func TestGenerate_HTTP405_JSONSchemaFallback(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// First call with json_schema should return 405
		if callCount == 1 {
			if respFormat, ok := reqBody["response_format"].(map[string]interface{}); ok {
				if respFormat["type"] == "json_schema" {
					w.WriteHeader(http.StatusMethodNotAllowed)
					_, _ = w.Write([]byte(`{"error":{"message":"Provider returned error","code":405,"metadata":{"raw":"{\"detail\":\"json_schema response format is not supported\"}"}}}`))
					return
				}
			}
		}

		// Second call with json_object should return 405
		if callCount == 2 {
			if respFormat, ok := reqBody["response_format"].(map[string]interface{}); ok {
				if respFormat["type"] == "json_object" {
					w.WriteHeader(http.StatusMethodNotAllowed)
					_, _ = w.Write([]byte(`{"error":{"message":"Provider returned error","code":405,"metadata":{"raw":"{\"detail\":\"json_object response format is not supported\"}"}}}`))
					return
				}
			}
		}
