...
import (
    ...
    "github.com/assagman/dsgo/signature_typed"
...
func main() {
    ...
    predictor, _ := signature_typed.NewPredict[SentimentInput, SentimentOutput](lm)
...
```go
predictor.WithOptions(&core.GenerateOptions{
    Temperature: 0.3,
    MaxTokens:   100,
})
```
...
```go
predictor.WithAdapter(core.NewJSONAdapter())
```
...
Run tests:
```bash
go test -v ./signature_typed/
```

## Example

Examples are not included in this layout.
